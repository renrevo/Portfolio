...
function initToptalBadge() {
  if (localStorage.getItem('toptal-badge-dismissed') === 'true') return;

  // Insert standard font CSS link if needed
  if (!document.querySelector('link[href*="use.typekit.net"]')) {
    const link = document.createElement('link');
    link.rel = 'stylesheet';
    link.href = 'https://use.typekit.net/kmj5qkr.css';
    document.head.appendChild(link);
  }

  const badge = document.createElement('div');
  badge.id = 'toptal-badge';
  badge.className = 'toptal-badge-floating';
  badge.innerHTML = `
    <button class="toptal-badge-close" aria-label="Dismiss badge">&times;</button>
    <div class="toptal-h">
      <div class="toptal-a">
  ...
  const closeBtn = badge.querySelector('.toptal-badge-close');
  if (closeBtn) {
    closeBtn.addEventListener('click', () => {
      badge.style.opacity = '0';
      badge.style.transform = window.innerWidth <= 768 ? 'scale(0.4) translateY(20px)' : 'scale(0.5) translateY(20px)';
      setTimeout(() => {
        badge.remove();
...
// Toptal Footer Social Badge
function initFooterToptalBadge() {
  const socialContainer = document.querySelector('.footer__social');
  if (!socialContainer) return;

  // Check if it's already there
  if (socialContainer.querySelector('.social-link-toptal')) return;

  const link = document.createElement('a');
  link.href = 'https://www.toptal.com/designers/resume/prateek-singh#gzKYlx';
  link.target = '_blank';
  link.className = 'social-link-toptal';
  link.setAttribute('aria-label', 'Toptal Profile');
  
  link.innerHTML = `
    <div class="toptal-badge-footer-wrap">
      <div class="toptal-h">
        <div class="toptal-a">
          <svg width="64" viewBox="0 0 60 17" xmlns="http://www.w3.org/2000/svg">
            <path d="m20.85 6.38 6.06-.89 2.72-5.49 2.71 5.49 6.06.89-4.39 4.28 1.04 6.03-5.42-2.85-5.43 2.85 1.04-6.03zm33.06 7.17 1.85-.27.82-1.67.83 1.67 1.84.27-1.33 1.31.31 1.83-1.65-.87-1.66.87.32-1.83zm-3.38-3.01-3.61-.52-1.61-3.26-1.62 3.26-3.6.52 2.6 2.55-.61 3.59 3.23-1.69 3.21 1.69-.61-3.59zm-45.19 3.01-1.85-.27-.82-1.67-.83 1.67-1.84.27 1.33 1.31-.31 1.83 1.65-.87 1.65.87-.31-1.83zm3.38-3.01 3.61-.52 1.61-3.26 1.61 3.26 3.61.52-2.6 2.55.61 3.59-3.23-1.69-3.22 1.69.62-3.59z" fill="#00c3ff"/>
